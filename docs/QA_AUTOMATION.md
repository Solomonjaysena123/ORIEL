...
Use `--category integration` to select a suite category, `--list` to inspect discovery without execution, and `--fail-on-flaky` in CI to reject tests that pass only after retry.
