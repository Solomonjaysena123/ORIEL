...
class RunnerAndReportTests(unittest.TestCase):
    ...
    def test_non_reconstructable_failure_does_not_crash_retry(self):
        class StatefulCase(unittest.TestCase):
            def __init__(self, method_name, required_state):
                super().__init__(method_name)
                self.required_state = required_state
            def run_case(self):
                self.fail(self.required_state)
        summary = QARunner(retries=2).run([StatefulCase("run_case", "expected")])
        self.assertEqual(summary.failed, 1)
        self.assertEqual(len(summary.results[0].attempts), 1)
