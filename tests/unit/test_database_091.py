...
class DatabaseFramework091Tests(unittest.TestCase):
    ...
    def test_legacy_migration_table_is_upgraded(self):
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder); schema = root / "schema.orl"; database = root / "legacy.db"
            schema.write_text(SCHEMA, encoding="utf-8")
            with closing(sqlite3.connect(database)) as connection:
                connection.execute(
                    "CREATE TABLE _oriel_migrations (id INTEGER PRIMARY KEY AUTOINCREMENT, source TEXT NOT NULL, applied_at TEXT DEFAULT CURRENT_TIMESTAMP)"
                )
                connection.execute("INSERT INTO _oriel_migrations(source) VALUES ('legacy.orl')")
                connection.commit()
            self.assertIsNone(migration_history(database)[0]["checksum"])
            self.assertEqual(migrate(schema, database), 1)
            self.assertEqual(len(migration_history(database)), 2)
