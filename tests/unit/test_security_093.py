...
from oriel.api_framework import Request, Response
...
class SecurityFramework093Tests(unittest.TestCase):
    def test_password_hashing_and_rehash_policy(self):
        ...
        hostile = encoded.replace("$100000$", "$999999999$")
        self.assertFalse(hasher.verify("correct horse battery staple", hostile))
    ...
    def test_signed_tokens_scope_expiry_and_tampering(self):
        ...
        with self.assertRaisesRegex(ValueError, "future"):
            service.verify(service.issue(Identity("user-1"), now=200), now=199)
    ...
    def test_sessions_revoke_and_expire(self):
        ...
        with self.assertRaisesRegex(ValueError, "positive"):
            store.create(Identity("user-3"), lifetime=0)
    ...
    def test_security_header_middleware(self):
        response = security_headers(Request("GET", "/"), lambda request: Response(body="ok"))
