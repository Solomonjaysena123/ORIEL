...
def diagnostics(text):
 ...
 if re.search(r'\bapi\s+\w+\s*\{',text): return []
 try: TypeChecker().check(Parser(Lexer(text).scan_tokens()).parse()); return []
 except OrielError as e:return [{"range":{"start":{"line":max(0,e.line-1),"character":max(0,e.column-1)},"end":{"line":max(0,e.line-1),"character":max(1,e.column)}},"severity":1,"code":e.code,"source":"oriel","message":e.message}]
 except Exception as e:return [{"range":{"start":{"line":0,"character":0},"end":{"line":0,"character":1}},"severity":1,"source":"oriel","message":str(e)}]
 ...
 while True:
  ...
  if method=='initialize':
   ...
   if method.endswith('hover'):
    value=f'**{name}** — ORIEL symbol' if name else '**ORIEL 0.4**'
