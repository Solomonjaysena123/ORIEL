...
from .web_framework import WebRequest, WebResponse
    ...
    def verify(self, password: str, encoded: str) -> bool:
        try:
            ...
            if algorithm != self.algorithm:
                return False
            actual = hashlib.pbkdf2_hmac("sha256", password.encode("utf-8"), _decode(salt), int(rounds))
...
class TokenService:
    ...
    def verify(self, token: str, *, now: int | None = None) -> Identity:
        ...
        if current >= int(payload.get("exp", 0)): raise ValueError("token expired")
        return Identity(str(payload["sub"]), tuple(payload.get("roles", ())), tuple(payload.get("permissions", ())))
...
class CSRFProtection:
    def __init__(self, secret: str):
        if len(secret) < 32: raise ValueError("CSRF secret must contain at least 32 characters")
...
def security_headers(request: WebRequest, next_handler: Callable[[WebRequest], WebResponse]) -> WebResponse:
    ...
    return WebResponse(response.body, response.status, headers)
